 ...
 * provider-independent. It can be used for raw secret keys that can be specified as <code>byte[]
 * </code>.
 */
@NullMarked
public class SecretKeySpec implements SecretKey, KeySpec, Serializable {
   ...
   * @throws IllegalArgumentException if the key data or the algorithm name is null or if the key
   *     data is empty.
   */
  public SecretKeySpec(byte @Nullable [] key, @Nullable String algorithm) {
    if (key == null) {
  ...
  /**
   * Creates a new <code>SecretKeySpec</code> for the key data from the specified buffer <code>key
   * </code> starting at <code>offset</code> with length <code>len</code> and the specified <code>
   * algorithm</code> name.
   *
   * @param key the key data.
   ...
   * @param algorithm the algorithm name.
   * @throws IllegalArgumentException if the key data or the algorithm name is null, the key data is
   *     empty or <code>offset</code> and <code>len</code> do not specify a valid chunk in the
   *     buffer <code>key</code>.
   * @throws ArrayIndexOutOfBoundsException if <code>offset</code> or <code>len</code> is negative.
   */
  public SecretKeySpec(byte @Nullable [] key, int offset, int len, @Nullable String algorithm) {
    if (key == null) {
      throw new IllegalArgumentException("key == null");
    }
    if (key.length == 0) {
  ...
  /**
   * Compares the specified object with this <code>SecretKeySpec</code> instance.
   *
   * @param obj the object to compare.
   * @return true if the algorithm name and key of both object are equal, otherwise false.
   */
  @Override
