...
public final class HttpCookie implements Cloneable {
   ...
   */
  public static boolean domainMatches(@Nullable String domainPattern, @Nullable String host) {
  ...
  private static String matchablePath(@Nullable String path) {
    if (path == null) {
      return "/";
  ...
  /**
   * Returns true if {@code s.substring(firstCharacter)} contains a dot between its first and last
   * characters, exclusive. This considers both {@code android.com} and {@code co.uk} to be fully
   * qualified domain names, but not {@code android.com.}, {@code .com}. or {@code android}.
   *
   * <p>Although this implements the cookie spec's definition of FQDN, it is not general purpose.
   * For example, this returns true for IPv4 addresses.
   */
  ...
  static class CookieParser {
    ...
    public List<HttpCookie> parse() {
      ...
      while (true) {
        ...
        while (true) {
           ...
           */
          String terminators =
              pre2965 || "expires".equals(attributeName) || "port".equals(attributeName)
                  ? ";"
                  : ";,";
    ...
    /**
     * Returns the next attribute name, or null if the input has been exhausted. Returns wth the
     * cursor on the delimiter that follows.
     */
    private @Nullable String readAttributeName(boolean returnLowerCase) {
      skipWhitespace();
    ...
    /**
     * Reads an attribute value, by parsing either a quoted string or until the next character in
     * {@code terminators}. The terminator character is not consumed.
     */
  ...
  private @Nullable String commentURL;
  ...
  // Internal copy constructor for cloning.
  HttpCookie(HttpCookie that) {
    this.comment = that.comment;
    this.commentURL = that.commentURL;
    this.discard = that.discard;
    this.domain = that.domain;
    this.maxAge = that.maxAge;
    this.name = that.name;
    this.path = that.path;
    this.portList = that.portList;
  ...
  public String toString() {
    ...
    StringBuilder result =
        new StringBuilder().append(name).append("=").append("\"").append(value).append("\"");
