...
public final class URI implements Comparable<URI>, Serializable {
  ...
  /**
   * Encodes the unescaped characters of {@code s} that are not permitted. Permitted characters are:
   *
   * <ul>
  ...
  private transient @Nullable String scheme;
  private transient @Nullable String schemeSpecificPart;
  private transient @Nullable String authority;
  private transient @Nullable String userInfo;
  private transient @Nullable String host;
  private transient int port = -1;
  private transient @Nullable String path;
  private transient @Nullable String query;
  private transient @Nullable String fragment;
  ...
  /**
   * Breaks uri into its component parts. This first splits URI into scheme, scheme-specific part
   * and fragment: [scheme:][scheme-specific part][#fragment]
   *
   * <p>Then it breaks the scheme-specific part into authority, path and query:
   * [//authority][path][?query]
   *
   * <p>Finally it delegates to parseAuthority to break the authority into user info, host and port:
   * [user-info@][host][:port]
   */
  private void parseURI(String uri, boolean forceServer) throws URISyntaxException {
    ...
    if (colon < UrlUtils.findFirstOf(uri, "/?#", 0, fragmentStart)) {
      ...
      if (!uri.regionMatches(start, "/", 0, 1)) {
        opaque = true;
        schemeSpecificPart =
            ALL_LEGAL_ENCODER.validate(uri, start, fragmentStart, "scheme specific part");
        return;
      }
    } else {
      absolute = false;
  ...
  /**
   * Breaks this URI's authority into user info, host and port parts. [user-info@][host][:port] If
   * any part of this fails this method will give up and potentially leave these fields with their
   * default values.
   *
   * @param forceServer true to always throw if the authority cannot be parsed. If false, this
   *     method may still throw for some kinds of errors; this unpredictable behavior is consistent
   *     with the RI.
   */
   ...
   *
   * @param forceServer true to always throw if the host cannot be parsed. If false, this method may
   *     still throw for some kinds of errors; this unpredictable behavior is consistent with the
   *     RI.
   */
  ...
  /**
   * Returns true if {@code first} and {@code second} are equal after unescaping hex sequences like
   * %F1 and %2b.
   */
  ...
  private @Nullable String decode(@Nullable String s) {
