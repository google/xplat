...
final class CookieStoreImpl implements CookieStore {
  ...
  private final Map<@Nullable URI, List<HttpCookie>> map =
      new HashMap<@Nullable URI, List<HttpCookie>>();

  public synchronized void add(@Nullable URI uri, HttpCookie cookie) {
    uri = cookiesUri(uri);
  ...
  private @Nullable URI cookiesUri(@Nullable URI uri) {
    if (uri == null) {
      return null;
    }
    try {
  ...
  public synchronized boolean remove(@Nullable URI uri, HttpCookie cookie) {
