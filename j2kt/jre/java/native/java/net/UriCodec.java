...
abstract class UriCodec {
      ...
      if ((ch >= 'a' && ch <= 'z')
        ...
        if (d1 == -1 || d2 == -1) {
          throw new URISyntaxException(
              uri, "Invalid % sequence: " + uri.substring(i, i + 3) + " in " + name, i);
        }
        i += 3;
      } else {
   ...
   *
   * @param isPartiallyEncoded true to fix input that has already been partially or fully encoded.
   *     For example, input of "hello%20world" is unchanged with isPartiallyEncoded=true but would
   *     be double-escaped to "hello%2520world" otherwise.
   */
  private void appendEncoded(
      StringBuilder builder, String s, Charset charset, boolean isPartiallyEncoded) {
    if (s == null) {
   ...
   * @param throwOnFailure true to throw an IllegalArgumentException on invalid escape sequences;
   *     false to replace them with the replacement character (U+fffd).
   */
