...
/**
 * This class provides an implementation of {@code FilterOutputStream} that compresses data using
 * the <i>DEFLATE</i> algorithm. Basically it wraps the {@code Deflater} class and takes care of the
 * buffering.
 *
 * @see Deflater
 */
...
public class DeflaterOutputStream extends FilterOutputStream {
  ...
  /**
   * Constructs a new instance with the given {@code Deflater} and flushing behavior (see {@link
   * #flush}).
   *
   * @since 1.7
   */
   ...
   *
   * @throws IOException If an error occurs during deflation.
   */
  ...
  /**
   * Writes any unwritten compressed data to the underlying stream, the closes all underlying
   * streams. This stream can no longer be used after close() has been called.
   *
   * @throws IOException If an error occurs while closing the data compression process.
   */
  @Override
  ...
  /**
   * Compresses {@code byteCount} bytes of data from {@code buf} starting at {@code offset} and
  ...
  public void write(byte[] buffer, int offset, int byteCount) throws IOException {
    ...
    checkCriticalArrayBounds(offset, offset + byteCount, buffer.length);
    if (!def.needsInput()) {
  ...
  /**
   * Flushes the underlying stream. This flushes only the bytes that can be compressed at the
   * highest level.
   *
