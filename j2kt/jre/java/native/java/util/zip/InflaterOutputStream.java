...
/**
 * An {@code OutputStream} filter to decompress data. Callers write compressed data in the "deflate"
 * format, and uncompressed data is written to the underlying stream.
 *
 * @since 1.6
 */
@NullMarked
public class InflaterOutputStream extends FilterOutputStream {
   ...
   * implementation-defined default internal buffer size. {@code out} is a destination for
   * uncompressed data, and compressed data will be written to this stream.
   *
  ...
  /**
   * Constructs an {@code InflaterOutputStream} with the given {@code Inflater} and given internal
   * buffer size. {@code out} is a destination for uncompressed data, and compressed data will be
  ...
  /**
   * Writes a byte to the decompressing output stream. {@code b} should be a byte of compressed
   * input. The corresponding uncompressed data will be written to the underlying stream.
   *
   * @param b the byte
   * @throws IOException if an I/O error occurs, or the stream has been closed
  ...
  /**
   * Writes to the decompressing output stream. The {@code bytes} array should contain compressed
   * input. The corresponding uncompressed data will be written to the underlying stream.
   *
   * @throws IOException if an I/O error occurs, or the stream has been closed
   * @throws ZipException if a zip exception occurs.
  ...
  public void write(byte[] bytes, int offset, int byteCount) throws IOException, ZipException {
    checkClosed();
    checkCriticalArrayBounds(offset, offset + byteCount, bytes.length);
