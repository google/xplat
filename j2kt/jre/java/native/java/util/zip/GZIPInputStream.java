...
/**
 * The {@code GZIPInputStream} class is used to read data stored in the GZIP format, reading and
 * decompressing GZIP data from the underlying stream into its buffer.
 *
 * <h3>Example</h3>
 *
 * <p>Using {@code GZIPInputStream} is easier than {@link ZipInputStream} because GZIP is only for
 * compression, and is not a container for multiple files. This code decompresses the data from a
 * GZIP stream, similar to the {@code gunzip(1)} utility.
 *
 * <pre>
 * InputStream is = ...
 ...
 * </pre>
 *
 * <p>Note that this class ignores all remaining data at the end of the last GZIP member.
 */
@NullMarked
public class GZIPInputStream extends InflaterInputStream {
  ...
  /**
   * Construct a {@code GZIPInputStream} to read from GZIP data from the underlying stream. Set the
   * internal buffer size to {@code size}.
   *
   * @param is the {@code InputStream} to read data from.
  ...
  public GZIPInputStream(InputStream is, int size) throws IOException {
    ...
    try {
      ...
      final short magic = readLittleEndianShort(header, 0);
  ...
  private boolean maybeReadNextMember() throws IOException {
    ...
    final short magic = readLittleEndianShort(buffer, 0);
    if (magic != (short) GZIP_MAGIC) {
  ...
  private static void parseGzipHeader(InputStream in, byte[] header, CRC32 crc, byte[] scratch)
      throws IOException {
    ...
    if ((flags & FEXTRA) != 0) {
      ...
      int length = readLittleEndianShort(header, 0) & 0xffff;
      while (length > 0) {
    ...
    if (hcrc) {
      readFully(in, header, 0, 2);
      short crc16 = readLittleEndianShort(header, 0);
  ...
  private void verifyCrc() throws IOException {
    ...
    if (readLittleEndianInt(b, 0) != (int) crc.getValue()) {
      throw new IOException("CRC mismatch");
    }
    if (readLittleEndianInt(b, 4) != inf.getTotalOut()) {
  ...
  private static void readFully(InputStream in, byte[] buffer, int offset, int length)
      throws IOException {
    int total = 0;
    while (total < length) {
