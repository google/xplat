 ...
 * <h3>Example</h3>
 *
 * <p>Using {@code GZIPOutputStream} is a little easier than {@link ZipOutputStream} because GZIP is
 * only for compression, and is not a container for multiple files. This code creates a GZIP stream,
 * similar to the {@code gzip(1)} utility.
 *
 * <pre>
 * OutputStream os = ...
 * byte[] bytes = ...
...
public class GZIPOutputStream extends DeflaterOutputStream {
  ...
  /** Constructs a new {@code GZIPOutputStream} to write data in GZIP format to the given stream. */
  public GZIPOutputStream(OutputStream os) throws IOException {
    this(os, BUF_SIZE, true);
  }

  /**
   * Constructs a new {@code GZIPOutputStream} to write data in GZIP format to the given stream with
   * the given flushing behavior (see {@link DeflaterOutputStream#flush}).
   *
   * @since 1.7
   */
  ...
  /**
   * Constructs a new {@code GZIPOutputStream} to write data in GZIP format to the given stream with
   * the given internal buffer size and flushing behavior (see {@link DeflaterOutputStream#flush}).
   */
  ...
  /**
   * Indicates to the stream that all data has been written out, and any GZIP terminal data can now
   * be written.
   *
  ...
  /**
   * Write up to nbytes of data from the given buffer, starting at offset off, to the underlying
