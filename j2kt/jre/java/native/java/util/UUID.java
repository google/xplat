 ...
 *
 * <p>There are multiple, variant layouts of UUIDs, but this class is based upon variant 2 of <a
 * href="http://www.ietf.org/rfc/rfc4122.txt">RFC 4122</a>, the Leach-Salz variant. This class can
 * be used to model alternate variants, but most of the methods will be unsupported in those cases;
...
public final class UUID implements Serializable, Comparable<UUID> {
  ...
  private static @Nullable SecureRandom rng;
  ...
  /**
   * Sets up the transient fields of this instance based on the current values of the {@code
   * mostSigBits} and {@code leastSigBits} fields.
   */
  private void init() {
    // setup hash field
  ...
  /**
   * Generates a variant 2, version 4 (randomly generated number) UUID as per <a
  ...
  private static UUID makeUuid(byte[] hash, int version) {
    long msb = peekLong(hash, 0);
    long lsb = peekLong(hash, 8);
  ...
  /** Reads 8 bytes from the array (at offset) and interprets them as a long (big endian). */
  private static long peekLong(byte[] bytes, int offset) {
    long result = 0;
    for (int i = 0; i < 8; i++) {
      result |= ((long) bytes[offset + 7 - i] & 0xff) << (i * 8);
    }
    return result;
  }
  ...
  /**
   * The version of the variant 2 UUID as per <a href="http://www.ietf.org/rfc/rfc4122.txt">RFC
   * 4122</a>. If the variant is not 2, then the version will be 0.
   *
   * <ul>
   *   <li>1 - Time-based UUID
   *   <li>2 - DCE Security UUID
   *   <li>3 - Name-based with MD5 hashing UUID ({@link #nameUUIDFromBytes(byte[])})
   *   <li>4 - Randomly generated UUID ({@link #randomUUID()})
   *   <li>5 - Name-based with SHA-1 hashing UUID
   * </ul>
   *
  ...
  /**
   * The variant of the UUID as per <a href="http://www.ietf.org/rfc/rfc4122.txt">RFC 4122</a>.
   *
   * <ul>
   *   <li>0 - Reserved for NCS compatibility
   *   <li>2 - RFC 4122/Leach-Salz
   *   <li>6 - Reserved for Microsoft Corporation compatibility
   *   <li>7 - Reserved for future use
   * </ul>
   *
   * @return an {@code int} value.
   */
  ...
  /**
   * Compares this UUID to the specified UUID. The natural ordering of UUIDs is based upon the value
   * of the bits from most significant to least significant.
   *
   * @param uuid the UUID to compare to.
   * @return a value of -1, 0 or 1 if this UUID is less than, equal to or greater than {@code uuid}.
   */
  ...
  /**
   * Compares this UUID to another object for equality. If {@code object} is not {@code null}, is a
   * UUID instance, and all bits are equal, then {@code true} is returned.
   *
   * @param object the {@code Object} to compare to.
   * @return {@code true} if this UUID is equal to {@code object} or {@code false} if not.
   */
  @Override
  ...
  /**
   * Returns a string representation of this UUID in the following format, as per <a
   * href="http://www.ietf.org/rfc/rfc4122.txt">RFC 4122</a>.
   *
   * <pre>
