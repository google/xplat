 ...
 *
 * <p>It is dangerous to seed {@code Random} with the current time because that value is more
 * predictable to an attacker than the default seed.
 *
...
public class Random implements Serializable {
  ...
  /**
   * Constructs a random generator with an initial state that is unlikely to be duplicated by a
  ...
  /**
   * Construct a random generator with the given {@code seed} as the initial state. Equivalent to
   * {@code Random r = new Random(); r.setSeed(seed);}.
   *
   * <p>This constructor is mainly useful for <i>predictability</i> in tests. The default
   * constructor is likely to provide better randomness.
   */
   ...
   *
   * <p>Subclasses only need to override this method to alter the behavior of all the public
   * methods.
   */
  ...
  /**
   * Returns a pseudo-random (approximately) normally distributed {@code double} with mean 0.0 and
   * standard deviation 1.0. This method uses the <i>polar method</i> of G. E. P. Box, M. E. Muller,
   * and G. Marsaglia, as described by Donald E. Knuth in <i>The Art of Computer Programming, Volume
   * 2: Seminumerical Algorithms</i>, section 3.4.1, subsection C, algorithm P.
   */
  ...
  /**
   * Modifies the seed using a linear congruential formula presented in <i>The Art of Computer
