...
/**
 * A cancellable asynchronous computation. This class provides a base implementation of {@link
 * Future}, with methods to start and cancel a computation, query to see if the computation is
 * complete, and retrieve the result of the computation. The result can only be retrieved when the
 * computation has completed; the {@code get} methods will block if the computation has not yet
 ...
 * Because {@code FutureTask} implements {@code Runnable}, a {@code FutureTask} can be submitted to
 * an {@link Executor} for execution.
 *
 * <p>In addition to serving as a standalone class, this class provides {@code protected}
...
public class FutureTask<V extends @Nullable Object> implements RunnableFuture<V> {
  ...
  private @Nullable Object outcome; // non-volatile, protected by state reads/writes

  /** The thread running the callable; CASed during run() */
  private final AtomicReference<@Nullable Thread> runner = new AtomicReference<>();

  /** Treiber stack of waiting threads */
  private final AtomicReference<@Nullable WaitNode> waiters = new AtomicReference<>();
  ...
  public FutureTask(Callable<V> callable) {
    ...
    this.state = new AtomicInteger(NEW); // ensure visibility of callable
  }
  ...
  public boolean cancel(boolean mayInterruptIfRunning) {
    if (!(state.get() == NEW
        && state.compareAndSet(NEW, mayInterruptIfRunning ? INTERRUPTING : CANCELLED)))
      return false;
  ...
  protected void setException(Throwable t) {
    if (state.compareAndSet(NEW, COMPLETING)) {
      outcome = t;
      state.set(EXCEPTIONAL); // final state
      finishCompletion();
    }
  }
  ...
  @Override
  public void run() {
    if (state.get() != NEW || !runner.compareAndSet(null, Thread.currentThread())) return;
    try {
  ...
  protected boolean runAndReset() {
    if (state.get() != NEW || !runner.compareAndSet(null, Thread.currentThread())) return false;
    boolean ran = false;
    int s = state.get();
    try {
      Callable<V> c = callable;
  ...
  private void handlePossibleCancellationInterrupt(int s) {
    ...
    if (s == INTERRUPTING)
      while (state.get() == INTERRUPTING) Thread.yield(); // wait out pending interrupt
  ...
  static final class WaitNode {
    ...
    volatile @Nullable WaitNode next;
  ...
  private int awaitDone(boolean timed, long nanos) throws InterruptedException {
    ...
    for (; ; ) {
      ...
      if (s > COMPLETING) {
        ...
        q = new WaitNode();
      } else if (!queued) queued = waiters.compareAndSet(q.next = waiters.get(), q);
      else if (timed) {
        ...
        if (state.get() < COMPLETING) LockSupport.parkNanos(this, parkNanos);
      ...
      for (; ; ) { // restart on removeWaiter race
        for (WaitNode pred = null, q = waiters.get(), s; q != null; q = s) {
          ...
          if (q.thread != null) pred = q;
            ...
            continue retry;
          } else if (!waiters.compareAndSet(q, s)) continue retry;
        }
        break;
      }
    }
