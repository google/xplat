 ...
 * with the underlying input stream is minimized, since most (smaller) requests can be satisfied by
 * accessing the buffer alone. The drawback is that some extra space is required to hold the buffer
 * and that copying takes place when flushing that buffer, but this is usually outweighed by the
...
public class BufferedOutputStream extends FilterOutputStream {
  ...
  /**
   * Constructs a new {@code BufferedOutputStream}, providing {@code out} with a buffer of 8192
   * bytes.
   *
  ...
  /**
   * Constructs a new {@code BufferedOutputStream}, providing {@code out} with {@code size} bytes of
   * buffer.
   *
   * @param out the {@code OutputStream} the buffer writes to.
  ...
  /**
   * Flushes this stream to ensure all pending data is written out to the target stream. In
   * addition, the target stream is flushed.
   *
  ...
  /**
   * Writes {@code count} bytes from the byte array {@code buffer} starting at {@code offset} to
   * this stream. If there is room in the buffer to hold the bytes, they are copied in. If not, the
   * buffered bytes plus the bytes in {@code buffer} are written to the target stream, the target is
  ...
  /**
   * Writes one byte to this stream. Only the low order byte of the integer {@code oneByte} is
   * written. If there is room in the buffer, the byte is copied into the buffer and the count
   * incremented. Otherwise, the buffer plus {@code oneByte} are written to the target stream, the
   * target is flushed, and the buffer is reset.
   *
