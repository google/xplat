...
/**
 * Parses a stream into a set of defined tokens, one at a time. The different types of tokens that
 * can be found are numbers, identifiers, quoted strings, and different comment styles. The class
 * can be used for limited processing of source code of programming languages like Java, although it
 * is nowhere near a full parser.
 */
@NullMarked
public class StreamTokenizer {
  /** Contains a number if the current token is a number ({@code ttype} == {@code TT_NUMBER}). */
  public double nval;

  /** Contains a string if the current token is a word ({@code ttype} == {@code TT_WORD}). */
  ...
  /**
   * After calling {@code nextToken()}, {@code ttype} contains the type of token that has been read.
   * When a single character is read, its value converted to an integer is stored in {@code ttype}.
   * For a quoted string, the value is the quoted character. Otherwise, its value is one of the
   * following:
   *
   * <ul>
   *   <li>{@code TT_WORD} - the token is a word.
   *   <li>{@code TT_NUMBER} - the token is a number.
   *   <li>{@code TT_EOL} - the end of line has been reached. Depends on whether {@code
   *       eolIsSignificant} is {@code true}.
   *   <li>{@code TT_EOF} - the end of the stream has been reached.
   * </ul>
   */
  ...
  private @Nullable InputStream inStream;

  private @Nullable Reader inReader;
  ...
  private StreamTokenizer() {
    ...
    /**
     * '/' is a comment character. Single quote '\'' and double quote '"' are string quote
     * characters.
     */
    commentChar('/');
    quoteChar('"');
    ...
    parseNumbers();
    /**
     * Ends of lines are treated as white space, not as separate tokens. C-style and C++-style
     * comments are not recognized. These are the defaults and are not needed in constructor.
     */
  }

  /**
   * Constructs a new {@code StreamTokenizer} with {@code is} as source input stream. This
   * constructor is deprecated; instead, the constructor that takes a {@code Reader} as an argument
   * should be used.
   *
  ...
  /**
   * Constructs a new {@code StreamTokenizer} with {@code r} as source reader. The tokenizer's
   * initial state is as follows:
   *
   * <ul>
   *   <li>All byte values 'A' through 'Z', 'a' through 'z', and '&#92;u00A0' through '&#92;u00FF'
   *       are considered to be alphabetic.
   *   <li>All byte values '&#92;u0000' through '&#92;u0020' are considered to be white space. '/'
   *       is a comment character.
   *   <li>Single quote '\'' and double quote '"' are string quote characters.
   *   <li>Numbers are parsed.
   *   <li>End of lines are considered to be white space rather than separate tokens.
  ...
  /**
   * Specifies whether the end of a line is significant and should be returned as {@code TT_EOF} in
   * {@code ttype} by this tokenizer.
   *
  ...
  /**
   * Specifies whether word tokens should be converted to lower case when they are stored in {@code
   * sval}.
   *
   * @param flag {@code true} if {@code sval} should be converted to lower case, {@code false}
   *     otherwise.
   */
  ...
  /**
   * Parses the next token from this tokenizer's source stream or reader. The type of the token is
   * stored in the {@code ttype} field, additional information may be stored in the {@code nval} or
   * {@code sval} fields.
   *
  ...
  public int nextToken() throws IOException {
    ...
    /** Check for digits before checking for words since digits can be contained within words. */
  ...
  /**
   * Specifies that the character {@code ch} shall be treated as an ordinary character by this
   * tokenizer. That is, it has no special meaning as a comment character, word component, white
   * space, string delimiter or number.
   *
  ...
  /**
   * Specifies that the characters in the range from {@code low} to {@code hi} shall be treated as
   * an ordinary character by this tokenizer. That is, they have no special meaning as a comment
   * character, word component, white space, string delimiter or number.
   *
  ...
  /**
   * Indicates that the current token should be pushed back and returned again the next time {@code
   * nextToken()} is called.
   */
  ...
  private int read() throws IOException {
    ...
    if (inStream == null) {
      if (inReader == null) {
        throw new NullPointerException("No source reader or stream specified");
      }
  ...
  /**
   * Specifies whether "slash-slash" (C++-style) comments shall be recognized. This kind of comment
   * ends at the end of the line.
   *
   * @param flag {@code true} if {@code //} should be recognized as the start of a comment, {@code
   *     false} otherwise.
   */
  ...
  /**
   * Specifies whether "slash-star" (C-style) comments shall be recognized. Slash-star comments
   * cannot be nested and end when a star-slash combination is found.
   *
   * @param flag {@code true} if {@code /*} should be recognized as the start of a comment, {@code
   *     false} otherwise.
   */
  ...
  /**
   * Specifies that the characters in the range from {@code low} to {@code hi} shall be treated as
   * whitespace characters by this tokenizer.
   *
  ...
  /**
   * Specifies that the characters in the range from {@code low} to {@code hi} shall be treated as
   * word characters by this tokenizer. A word consists of a word character followed by zero or more
