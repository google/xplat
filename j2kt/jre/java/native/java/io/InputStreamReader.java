...
public class InputStreamReader extends Reader {
   ...
   * is used to decode bytes into characters is identified by name by {@code charsetName}. If the
   * encoding cannot be found, an UnsupportedEncodingException error is thrown.
   *
   ...
   * @throws UnsupportedEncodingException if the encoding specified by {@code charsetName} cannot be
   *     found.
   */
  ...
  /**
   * Constructs a new InputStreamReader on the InputStream {@code in} and CharsetDecoder {@code
   * dec}.
   *
   ...
   * or null if this writer has been closed. Most callers should probably keep track of the String
   * or Charset they passed in; this method may not return the same name.
   */
  ...
  /**
   * Reads up to {@code count} characters from this reader and stores them at position {@code
   * offset} in the character array {@code buffer}. Returns the number of characters actually read
   * or -1 if the end of the reader has been reached. The bytes are either obtained from converting
   * bytes in this reader's buffer or by first filling the buffer from the source InputStream and
   * then reading from the buffer.
   *
   * @throws IndexOutOfBoundsException if {@code offset < 0 || count < 0 || offset + count >
   *     buffer.length}.
   * @throws IOException if this reader is closed or some other I/O error occurs.
   */
  @Override
