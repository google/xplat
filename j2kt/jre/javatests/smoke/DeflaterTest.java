...
public class DeflaterTest extends TestCase {
  ...
  private final byte[] compressed = new byte[32];
  private final byte[] decompressed = new byte[20];
  ...
  private void deflateInflate(int flush) throws DataFormatException {
    int lastDeflated =
        deflater.deflate(compressed, totalDeflated, compressed.length - totalDeflated, flush);
    assertTrue(inflater.needsInput());
    inflater.setInput(compressed, totalDeflated, lastDeflated);
    totalDeflated += lastDeflated;
    totalInflated +=
        inflater.inflate(decompressed, totalInflated, decompressed.length - totalInflated);
  }
