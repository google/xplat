...
public final class URITest extends TestCase {
  ...
  public void testUriParts() throws Exception {
    ...
    assertEquals(
        "//username:password@host:8080/directory/file?query", uri.getRawSchemeSpecificPart());
  }

  public void testEqualsCaseMapping() throws Exception {
    assertEquals(
        new URI("HTTP://localhost/foo?bar=baz#quux"), new URI("HTTP://localhost/foo?bar=baz#quux"));
    assertEquals(
        new URI("http://localhost/foo?bar=baz#quux"), new URI("http://LOCALHOST/foo?bar=baz#quux"));
    assertFalse(
  ...
  // Does not pass when running on JVM
  public void ignore_testRfc1808AbnormalExampleTooManyDotDotSequences() throws Exception {
    URI base = new URI("http://a/b/c/d;p?q");
  ...
  // Does not pass when running on JVM
  public void ignore_testRelativize() throws Exception {
  ...
  public void testParseServerAuthorityOmittedAuthority() throws Exception {
    ...
    URI unused = uri.parseServerAuthority(); // does nothing!
  ...
  public void testEncodingParts() throws Exception {
    URI uri =
        new URI("http", "user:pa55w?rd", "host", 80, "/doc|search", "q=green robots", "over 6\"");
    assertEquals("http", uri.getScheme());
    ...
    assertEquals(
        "//user:pa55w%3Frd@host:80/doc%7Csearch?q=green%20robots", uri.getRawSchemeSpecificPart());
    assertEquals(
        "http://user:pa55w%3Frd@host:80/doc%7Csearch?q=green%20robots#over%206%22", uri.toString());
  }
