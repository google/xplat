...
public class HttpCookieTest extends TestCase {
  ...
  public void test_HttpCookie_LString_LString() {
    ...
    // constructHttpCookie("Comment", value); // Fails on RI
    // constructHttpCookie("CommentURL", value); // Fails on RI
    // constructHttpCookie("Domain", value);  // Fails on RI
    // constructHttpCookie("Discard", value); // Fails on RI
    // constructHttpCookie("Max-Age", value); // Fails on RI
    // constructHttpCookie("  Path     ", value); // Fails on RI
    // constructHttpCookie("Port  ", value); // Fails on RI
    // constructHttpCookie("SeCure", value); // Fails on RI
    // constructHttpCookie("VErsion", value); // Fails on RI
    // constructHttpCookie("expires", value); // Fails on RI
  ...
  private static void constructHttpCookie(String name, @Nullable String value) {
    try {
  ...
  public void test_DomainMatches() {
    ...
    // TODO: b/377734324 - Re-enable this test
    // JDK-7023713
  ...
  public void test_Get_SetDiscard() {
    HttpCookie cookie = new HttpCookie("cookie'sName", "cookie's Test value");
  ...
  public void test_HasExpired() {
    HttpCookie cookie = new HttpCookie("cookie'\"sName123456", "cookie's Test?()!@# value");
    ...
    cookie.setMaxAge(Long.MIN_VALUE);
    cookie.setDiscard(false);
    // assertTrue(cookie.hasExpired());  // Fails on RI
  ...
  //     String futureExpiryDate = dateFormat.format(new Date(System.currentTimeMillis() +
  // 100_000));
  ...
  public void test_ToString() {
    ...
    cookie.setPath(null);
    assertEquals("test=\"testValue\";$Domain=\".b.com\";$Port=\"80.562Ab\"", cookie.toString());
  ...
  public void test_Parse_exception() {
    ...
    // checkInvalidCookie("Set-Cookie2:COmmentURL=\"lala\"");  // Fails on RI
  ...
  public void test_Parse() {
    ...
    list =
        HttpCookie.parse(
            "Set-cookie2:test=null\";;Port=abde,82;Path=/temp;;;Discard;commentURl=http://harmonytest.org;Max-age=-10;");
    cookie = list.get(0);
    assertEquals("null\"", cookie.getValue());
    assertEquals(1, cookie.getVersion());
    ...
    list =
        HttpCookie.parse(
            "Set-Cookie:name=tes,t;Comment=version1-cookie;Discard=false;commentURL=vers\n"
    ...
    list = HttpCookie.parse("Set-Cookie:name=tes,t;Comment=sentence;Comment=anotherSentence");
    cookie = list.get(0);
    assertEquals("sentence", cookie.getComment());

    // Check CommentURL
    list = HttpCookie.parse("Set-Cookie:name=tes,t;Commenturl;commentuRL=(la,la)");
    cookie = list.get(0);
    ...
    list = HttpCookie.parse("Set-Cookie2:name=test;,Set-Cookie2:name2=test2;comment=c234;");
    cookie = list.get(0);
    assertEquals("name", cookie.getName());
    assertEquals(1, cookie.getVersion());
    ...
    HttpCookie c = HttpCookie.parse("Set-cookie:NAME2=VALUE2;path=/t;domain=.b.c;version=1").get(0);
    assertEquals(1, c.getVersion());

    c = HttpCookie.parse("Set-cookie2:NAME2=VALUE2;path=/t;domain=.b.c;version=0").get(0);
    assertEquals(1, c.getVersion());
    ...
    try {
      list = HttpCookie.parse("Set-Cookie:a  name=tes,t;Commenturl;commentuRL=(la,la);path=hello");
      fail("IllegalArgumentException expected");
    } catch (IllegalArgumentException expected) {
    }

    list = HttpCookie.parse("Set-Cookie:name=tes,t;Commenturl;commentuRL=(la,la);commentuRL=hello");
    cookie = list.get(0);
    assertEquals("(la,la)", cookie.getCommentURL());

    list = HttpCookie.parse("Set-Cookie:name=tes,t;Commenturl;commentuRL=(la,la); path  =hello");
    cookie = list.get(0);
    assertEquals("(la,la)", cookie.getCommentURL());
    assertEquals("hello", cookie.getPath());

    try {
      list = HttpCookie.parse("a  Set-Cookie:name=tes,t;Commenturl;commentuRL=(la,la);path=hello");
      fail("IllegalArgumentException expected");
    } catch (IllegalArgumentException expected) {
    }
  }
  ...
  public void test_Parse_versionConflict() {
    ...
    List<HttpCookie> list = HttpCookie.parse("Set-Cookie2:name=;expires=;discard");
    HttpCookie cookie = list.get(0);
    assertEquals(0, cookie.getVersion());
    assertTrue(cookie.getDiscard());
    ...
    list =
        HttpCookie.parse(
            "Set-Cookie:name=value;max-age=1000;expires=Tue, 17-Jan-1998 19:14:07 GMT;version=1");
    cookie = list.get(0);
    assertEquals(0, cookie.getVersion());
    assertEquals(1000, cookie.getMaxAge());
    assertFalse(cookie.hasExpired());
    ...
    list =
        HttpCookie.parse(
            "Set-Cookie2:name=value;max-age=1000;version=1;expires=Tue, 17-Jan-1998 19:07:14 GMT;");
    cookie = list.get(0);
    assertEquals(0, cookie.getVersion());
    assertEquals(1000, cookie.getMaxAge());
    assertFalse(cookie.hasExpired());
    ...
    list =
        HttpCookie.parse(
            "Set-Cookie2: name=value;expires=Sun, 27-Jan-2018 19:14:07"
                + " GMT;comment=mycomment;port=80,8080");
    cookie = list.get(0);
    assertEquals(0, cookie.getVersion());
    ...
    list =
        HttpCookie.parse(
            "Set-Cookie:name=test;comment=mycomment;commentURL=url;discard;domain=a.b.com;path=temp;port=79;secure");
    cookie = list.get(0);
    assertEquals(0, cookie.getVersion());
  }
  ...
  // test_Parse_multipleThreads disabled for J2kt (no Thread class available)
  // /**
  ...
  private void checkValidValue(String header, String value) {
    List<HttpCookie> list = HttpCookie.parse(header + "name=" + value + ";");
    HttpCookie cookie = list.get(0);
    assertEquals(value, cookie.getValue());
  }
  ...
  protected void setUp() throws Exception {
    ...
    // Locale.setDefault(Locale.ENGLISH); // Not available in J2kt
  }

  @Override
  protected void tearDown() throws Exception {
    // Locale.setDefault(locale); // Not available in J2kt
    super.tearDown();
  }
}
